...
function UserProfileContext() {
    ...
    const fetchRecord = () => {
        axiosConn
            ...
            .then((res) => {
                const fetchedData = res.data?.data?.userContext || [];
    ...
    return (<div>
            <Card className="my-4 rounded-none shadow-md">
                <CardHeader className="bg-muted/50">
                    <CardTitle className="text-lg font-normal">Contexts</CardTitle>
                </CardHeader>
                <Separator/>
                <CardContent>
                            ...
                            {(fieldTypeMap[a?.fielddetail?.fieldType]?.tag == 'multiselect' ?
                                <div className="flex flex-col gap-2 my-4">
                                    ...
                                    <MultiSelectField
                                        ...
                                        handleUpdate={(val) => {
                                            console.log(val);
                                            handleRecordContextInputChange(index, val)
                                        }}
                                    />
                                    {errors[index] && <p className="text-red-500 text-sm">{errors[index]}</p>}
