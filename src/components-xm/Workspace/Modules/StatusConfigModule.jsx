...
function StatusConfigModule({ value, onChange, isEditable = true }) {
    ...
    const fetchSelectedConfig = () => {
        axiosConn
            .post("http://localhost:3000/searchRecord", {
                ...
                offset: 0,
                getThisData: {
                    datasource: "StatusConfiguration",
                    attributes: [],
                    where: {
                        workspaceId: WorkspaceId,
                        orgId: localStorage.getItem("currentOrg"),
                        statusConfigurationId: value,
                    },
                },
            })
            .then((res) => {
                ...
                getThisData: {
                    ...
                    where: {
                        ...
                        ...(query && query.length > 0 && {
                            statusConfigurationName: { "$like": `%${query}%` },
                        }),
                    },
                },
            })
    ...
    const handleSelection = (newValue) => {
        ...
        onChange(newValue? parseInt(newValue?.statusConfigurationId) : null); // Update React Hook Form state
    ...
    return (
        <Popover open={labelDropOpen} onOpenChange={setLabelDropOpen}>
            <PopoverTrigger asChild>
                ...
                >
                    {defaultConfig!= null ? defaultConfig?.statusConfigurationName : 'NONE'}
                    ...
                    <CommandList>
                        ...
                        <CommandGroup>
                            ...
                            {filteredLabels?.map((labelItem) => (
                                <CommandItem
                                    key={labelItem.statusConfigurationId}
                                    value={labelItem.statusConfigurationName}
                                    onSelect={() => handleSelection(labelItem)}
                                >
                                    {labelItem.statusConfigurationName}
                                </CommandItem>
                            ))}
                        </CommandGroup>
                    </CommandList>
...
export  default StatusConfigModule;