...
function UserStatusSearchModule({ value, onChange, isEditable = true }) {
    ...
    useEffect(() => {
        axiosConn.post("http://localhost:3000/searchRecord", {
            limit: 10, offset: 0, getThisData: {
                datasource: "UserStatus", attributes: [], where: {
    ...
    useEffect(() => {
        if (query?.trim()?.length > 0) {
            axiosConn.post("http://localhost:3000/searchRecord", {
                limit: 10, offset: 0, getThisData: {
                    datasource: "UserStatus", attributes: [], where: {
                        workspaceId: WorkspaceId, orgId: localStorage.getItem("currentOrg"), userStatusName: {
                            $like: `%${query}%`,
                        },
                    }
                },
    ...
    const handleTagUpdate = (labelItem) => {
        if (!label.some(l => l.userStatusId === labelItem.userStatusId)) {
            setLabel(prevLabels => {
                const newLabels = [...prevLabels, labelItem];
                onChange(newLabels.map(a => a.userStatusId)); // Ensure the new state is passed
                return newLabels;
            });
        } else {
    ...
    const handleTagDelete = (labelItem) => {
        setLabel(prev => {
            const newLabels = prev.filter(l => l.userStatusId !== labelItem.userStatusId);
            onChange(newLabels.map(a => a.userStatusId)); // Ensure the new state is passed
            return newLabels;
        });
    };
                            ...
                            <CommandList>
                                ...
                                <CommandGroup>
                                    {filteredLabels.map((labelItem) => (<CommandItem
                                        key={labelItem.userStatusId}
                                        value={labelItem.userStatusName}
                                        onSelect={() => {
                                            handleTagUpdate(labelItem)
                                            // setLabel(prevLabels => {
                                            //     if (!prevLabels.some(l => l.userStatusId === labelItem.userStatusId)) {
                                            //         return [...prevLabels, labelItem];
                                            //     }
                                            //     return prevLabels;
                                            // });
                                            // if (!label.some(l => l.userStatusId === label.userStatusId)) {
                                            //     setLabel((prevLabels) => [...prevLabels, label]);
                                            // }
                                            setLabelDropOpen(false);
                                        }}
                                    >
                                        {labelItem.userStatusName}
                                    </CommandItem>))}
                                </CommandGroup>
                            </CommandList>
                        </Command>
...
export default UserStatusSearchModule;