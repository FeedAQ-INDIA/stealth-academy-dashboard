...
function TagSearchModule({initialValue}) {
    ...
    useEffect(() => {
        axiosConn.post("http://localhost:3000/searchRecord", {
            limit: 10, offset: 0, getThisData: {
                datasource: "Tags", attributes: [], where: {
    ...
    useEffect(() => {
        if (query?.trim()?.length > 0) {
            axiosConn.post("http://localhost:3000/searchRecord", {
                limit: 10, offset: 0, getThisData: {
                    datasource: "Tags", attributes: [], where: {
                        workspaceId: WorkspaceId, orgId: localStorage.getItem("currentOrg"), tagName: {
                            $like: `%${query}%`,
                        },
                    }
                },
    ...
    const handleTagUpdate = (labelItem) => {
        if (!label.some(l => l.tagId === labelItem.tagId)) {
            axiosConn.post("http://localhost:3000/addRecordStakeholderTags", {
                workspaceId: WorkspaceId, orgId: localStorage.getItem("currentOrg"),
                ...(RecordId && {recordId:RecordId}),
                ...(StakeholderId && {stakeholderId:StakeholderId}),
                tagIdList: [labelItem.tagId]
            }).then(res => {
    ...
    const handleTagDelete = (labelItem) => {
        if (labelItem) {
            axiosConn.post("http://localhost:3000/deleteRecordStakeholderTags", {
                workspaceId: WorkspaceId, orgId: localStorage.getItem("currentOrg"),
                ...(RecordId && {recordId:RecordId}),
                ...(StakeholderId && {stakeholderId:StakeholderId}),
                tagIdList: [labelItem.tagId]
                ...
                console.log(res?.data);
                setLabel(prev => prev.filter(l => l.tagId !== labelItem.tagId))
            }).catch(err => {
                console.log(err)
            })
        }else{
            ...
            <div className="flex max-w-screen flex-col items-start justify-between border px-4 py-3">
                <p className="text-sm font-medium leading-none flex flex-wrap gap-2 items-center">
                    {label?.map((a) => (<Badge
                        ...
                        >
                        <span className="text-xs text-white font-medium rounded-sm bg-gradient-to-r from-purple-500 to-pink-500 py-0.5 px-1 ">AI GEN</span>
