...
function LayoutSearchModule({ value, onChange, isEditable = true }) {
    ...
    //             setDefaultFilteredLabels(res.data?.data?.results)
    //         }).catch(err => {
    //             console.log(err)
    //         });
    //     }
    ...
    useEffect(() => {
        if (query?.trim()?.length > 0) {
            axiosConn.post("http://localhost:3000/searchRecord", {
                limit: 10, offset: 0, getThisData: {
                    datasource: "Layout", attributes: [], where: {
                        workspaceId: WorkspaceId, orgId: localStorage.getItem("currentOrg"), layoutName: {
                            $like: `%${query}%`,
                        },
                    }
                },
    ...
    const handleTagUpdate = (labelItem) => {
        if (!label.some(l => l.layoutId === labelItem.layoutId)) {
            setLabel(prevLabels => {
                const newLabels = [...prevLabels, labelItem];
                onChange(newLabels.map(a => a.layoutId)); // Ensure the new state is passed
                return newLabels;
            });
        } else {
    ...
    const handleTagDelete = (labelItem) => {
        setLabel(prev => {
            const newLabels = prev.filter(l => l.layoutId !== labelItem.layoutId);
            onChange(newLabels.map(a => a.layoutId)); // Ensure the new state is passed
            return newLabels;
        });
    };
                                ...
                                <CommandList>
                                    ...
                                    <CommandGroup>
                                        {filteredLabels.map((labelItem) => (<CommandItem
                                                key={labelItem.layoutId}
                                                value={labelItem.layoutName}
                                                onSelect={() => {
                                                    handleTagUpdate(labelItem)
                                                    // setLabel(prevLabels => {
                                                    //     if (!prevLabels.some(l => l.layoutId === labelItem.layoutId)) {
                                                    //         return [...prevLabels, labelItem];
                                                    //     }
                                                    //     return prevLabels;
                                                    // });
                                                    // if (!label.some(l => l.layoutId === label.layoutId)) {
                                                    //     setLabel((prevLabels) => [...prevLabels, label]);
                                                    // }
                                                    setLabelDropOpen(false);
                                                }}
                                            >
                                                {labelItem.layoutName}
                                            </CommandItem>))}
                                    </CommandGroup>
                                </CommandList>
                            </Command>
...
export default LayoutSearchModule;