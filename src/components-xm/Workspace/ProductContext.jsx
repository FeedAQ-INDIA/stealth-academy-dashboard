...
function ProductContext({contextConfigId}) {
    ...
    useEffect(() => {
        ...
        fetchRecord();
        fetchContextConfig();
    }, [ProductId, contextConfigId]);

    const fetchRecord = () => {
        axiosConn
            .post("http://localhost:3000/searchRecord", {
                limit: 10, offset: 0, getThisData: {
                    datasource: "ProductContext", attributes: [], where: {productId: ProductId}, include: [{
    ...
    const handleContextSavingToServer = () => {
        ...
        const updatedRecordList = recordContextDetail
            ...
            .map((a) => ({
                ...
                orgId: localStorage.getItem("currentOrg"), ...(ProductId && {productId: ProductId}),
                fieldId: a?.fieldId,
                fieldValue: a?.fieldValue,
                contextId: a?.productContextId,
            }));
