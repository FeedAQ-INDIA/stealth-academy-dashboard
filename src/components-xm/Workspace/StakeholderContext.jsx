...
function  StakeholderContext({contextConfigId}) {
    ...
    useEffect(() => {
        console.log('context id :: ', contextConfigId)
        fetchRecord();
        fetchContextConfig();
    }, [StakeholderId, WorkspaceId, contextConfigId]);

    const fetchRecord = () => {
        axiosConn
            .post("http://localhost:3000/searchRecord", {
                limit: 10, offset: 0, getThisData: {
                    datasource: "StakeholderContext", attributes: [], where: {stakeholderId: StakeholderId}, include: [{
    ...
    const handleContextSavingToServer = () => {
        ...
        const updatedRecordList = recordContextDetail
            ...
            .map((a) => ({
                ...
                contextId: a?.stakeholderContextId,
            }));
    ...
    return (<div>
        {recordContextDetail && recordContextDetail.length > 0 &&  <Card className="my-4 rounded-none shadow-md">
