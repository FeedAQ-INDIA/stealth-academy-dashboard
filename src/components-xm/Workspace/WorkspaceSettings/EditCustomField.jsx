...
function EditCustomField({initialValue, onFieldUpdate, fullFieldList}) {
    ...
    const fieldConfigSchema = z.object({
        fieldStatus: z.enum(["ACTIVE", "INACTIVE", "ARCHIVED"]),
        fieldEntityType: z.string().optional(),
        ...
        fieldToolTipText: z.string().min(3, "Tooltip is required"),
    ...
    useEffect(() => {
        if (initialValue) {
            form.reset({
                fieldStatus: initialValue?.fieldStatus,
                fieldEntityType: initialValue?.fieldEntityType,
                ...
                fieldToolTipText: initialValue?.fieldToolTipText,
    ...
    async function onFieldSubmit  (data, event) {
        ...
        const updatedField = {
            ...
            fieldEntityType: data.fieldEntityType,
            fieldLabel: data.fieldLabel,
            fieldKey: data.fieldKey,
            fieldType: data.fieldType,
            fieldOption: optionTextList.join(' | '),
            fieldToolTipText: data.fieldToolTipText,
    ...
    const checkFieldKeyAvailability = async (fieldKey) => {
        ...
        if(!initialValue?.workspaceFieldId){
            isDuplicateLocally = fullFieldList.some(a =>
                a.fieldKey === fieldKey
            );
        }else{
            isDuplicateLocally = fullFieldList.some(a =>
                a.fieldKey === fieldKey && a.workspaceFieldId !== initialValue?.workspaceFieldId
            );
        }
        ...
        try {
            const response = await axiosConn.post(`http://localhost:3000/checkFieldKeyAvailability`, {
                workspaceId: WorkspaceId,
                orgId: localStorage.getItem('currentOrg'),
                ...(ContextConfigurationId && { contextConfigId: ContextConfigurationId }),
                ...(initialValue?.workspaceFieldId && { fieldId: initialValue.workspaceFieldId }),
                fieldKey
            });
                        ...
                        <CardContent>
                            <div className="py-4">
                                ...
                                <div className="grid w-full   items-center gap-3 my-4">
                                    <FormField
                                        control={form.control}
                                        name="fieldToolTipText"
                                        render={({field}) => (<FormItem>
                                                <FormLabel>Field Tooltip Text</FormLabel>
                                                <FormControl>
                                                    <Textarea
                                                        placeholder="Type your Tooltip Text here."
                                                        {...field}
                                                    />
                                                </FormControl>
                                                <FormMessage/>
                                    ...
                                    <FormField
                                        ...
                                        render={({field}) => (<FormItem>
                                                ...
                                                <FormControl>
                                                    ...
                                                    >
                                                        <SelectTrigger>
                                                            <SelectValue placeholder="Select a Field Status"/>
                                                        </SelectTrigger>
                                                        <SelectContent>
