    ...
    const comprehensionData = {
        ...
        questions: [
            ...
            },
            {
                ...
                question: 'Which of the following are consequences of climate change mentioned in the passage? (Select all that apply)',
                options: [
                    'Rising sea levels',
            ...
            },
            {
                id: 4,
                type: 'audio',
                question: 'Record yourself explaining three consequences of climate change mentioned in the passage.'
            }
        ]
    };
    ...
    const submitAnswers = () => {
        ...
        const unansweredQuestions = comprehensionData.questions.filter(
            (question, index) => {
                ...
                return !answers.hasOwnProperty(index) || answer === '' || answer === null || answer === undefined;
            }
        );
                    ...
                    <Card className="hover:shadow-lg transition-shadow">
                        <CardHeader className="pb-4">
                            <div className="flex items-center gap-2">
                                ...
                                <span className="text-gray-600 text-sm">Question {questionIndex + 1}</span>
                            </div>
                        ...
                        <CardContent>
                            <RadioGroup 
                                ...
                                onValueChange={(value) => handleMCQAnswer(questionIndex, parseInt(value))}
                                className="space-y-3"
                            >
                                {question.options.map((option, optionIndex) => (
                                    ...
                                    >
                                        ...
                                        <Label htmlFor={`q${questionIndex}-${optionIndex}`} className="text-gray-700 cursor-pointer flex-1">
                                            {option}
                                        </Label>
                                    </div>
                                ))}
                ...
                return (
                    <Card className="hover:shadow-lg transition-shadow">
                        <CardHeader className="pb-4">
                            ...
                            <CardTitle className="text-lg font-semibold text-gray-800">{question.question}</CardTitle>
                            <p className="text-sm text-gray-600 mt-2">Select all correct answers</p>
                        </CardHeader>
                        <CardContent>
                                ...
                                {question.options.map((option, optionIndex) => (
                                    ...
                                    >
                                        <Checkbox
                                            ...
                                            onCheckedChange={() => handleMultipleChoiceAnswer(questionIndex, optionIndex)}
                                        />
                                        <Label htmlFor={`q${questionIndex}-${optionIndex}`} className="text-gray-700 cursor-pointer flex-1">
                                            {option}
                                        </Label>
                            ...
                            {selectedAnswers.length > 0 && (
                                <div className="mt-4 p-3 bg-blue-50 rounded-lg border border-blue-200">
                                    <div className="flex items-center gap-2 text-sm text-blue-800">
                                        <span>✓</span>
                                        <span>{selectedAnswers.length} option{selectedAnswers.length !== 1 ? 's' : ''} selected</span>
                                    </div>
                                </div>
                            )}
                        </CardContent>
                    ...
                    <Card className="hover:shadow-lg transition-shadow">
                        ...
                        <CardContent className="space-y-4">
                            <Textarea
                                ...
                                onChange={(e) => handleWrittenAnswer(questionIndex, e.target.value)}
                            ...
                            {isRecording && (
                                <div className="bg-red-50 border border-red-200 rounded-lg p-3 space-y-2">
                                    <div className="flex items-center justify-between">
                                        ...
                                        <span className="text-xs text-gray-600">Level: {Math.round(audioLevel)}%</span>
                                    </div>
                                    ...
                                    <div className="relative w-full h-2 bg-gray-200 rounded-full overflow-hidden">
                                        <div 
                                            className={`h-full transition-all duration-150 rounded-full ${
                                                audioLevel < 3 ? 'bg-red-400' :
                                                audioLevel < 15 ? 'bg-yellow-400' :
                                                audioLevel > 80 ? 'bg-orange-400' :
                                                'bg-green-400'
                                            }`}
                            ...
                            {audioBlob && (
                                <div className="bg-green-50 border border-green-200 rounded-lg p-3 space-y-2">
                                    <div className="flex items-center justify-between">
                                        ...
                                        <span className="text-xs text-green-700">Duration: {formatTime(recordingTime)}</span>
                                    </div>
                                    ...
                                    <div className="flex items-center justify-between">
                                        <div className="flex items-center gap-3 text-xs text-gray-600">
                                            <span>Quality: <span className={`font-medium ${getQualityColor(quality).split(' ')[0]}`}>{quality}</span></span>
                                ...
                                <div className="bg-blue-50 border border-blue-200 rounded-lg p-3">
                                    <div className="flex items-start gap-2">
                                        ...
                                        <div className="text-xs text-blue-800">
                                            ...
                                            <p>• Quiet environment • Clear speech • 30s-2min duration</p>
                                        </div>
                                    </div>
                                </div>
                            )}
            ...
            <div className="  mx-auto">
                                ...
                                <div className="flex items-center gap-4">
                                    <Progress 
                                        value={(
                                            ...
                                            }).length / comprehensionData.questions.length
                                        ) * 100} 
                                        className="w-32"
                                    />
                                    <Badge variant="secondary" className="bg-blue-100 text-blue-800">
                                        {comprehensionData.questions.filter((question, index) => {
                                            ...
                                            return answer !== undefined && answer !== '' && answer !== null;
                                        }).length} / {comprehensionData.questions.length} Answered
                                    </Badge>
                                </div>
                                {/* <Button 
                ...
                <div className="grid grid-cols-1   gap-8">
                        ...
                        <Card className="shadow-lg">
                            <CardHeader>
                                <CardTitle className="flex items-center gap-3">
                                    <div className="bg-indigo-100 text-indigo-800 p-2 rounded-lg  tracking-wide">📖</div>
                                    Reading Passage
                                </CardTitle>
                            </CardHeader>
                            <CardContent>
                    ...
                    <div className="space-y-4">
                        <Card>
                           <CardHeader  className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4"> <CardTitle className="text-xl font-bold text-gray-800 flex items-center gap-3 tracking-wide">
                                <div className="bg-green-100 text-green-800 p-2 rounded-lg">❓</div>
                                Questions
                            </CardTitle>
                            <Badge variant="outline" className="self-start sm:self-center">
                                Progress: {comprehensionData.questions.filter((question, index) => {
                                    const answer = answers[index];
                                    if (question.type === 'multiple-choice') {
                                        return answer && Array.isArray(answer) && answer.length > 0;
                                    }
                        ...
                        <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
                            <div className="text-gray-600 text-center sm:text-left">
                                <p className="text-sm">
                                    ...
                                    You can review and modify your answers anytime.
                                </p>
                            </div>
                            <Button 
                                onClick={submitAnswers}
